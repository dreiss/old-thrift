...
void t_c_generator::init_generator() {
  ...
  // Open namespace
  ns_open_ = namespace_open(program_->get_namespace("c"));
  ns_close_ = namespace_close(program_->get_namespace("c"));

  f_types_ <<
    "/* namespace, if any */" << endl << 
    ns_open_ << endl <<
    endl;
 ...
 */
void t_c_generator::generate_consts(std::vector<t_const*> consts) {
 ...
 */
void t_c_generator::print_const_value(ofstream& out, string name, t_type* type, t_const_value* value) {
 ...
 *
 */
string t_c_generator::render_const_value(ofstream& out, string name, t_type* type, t_const_value* value) {
 ...
 */
void t_c_generator::generate_c_struct(t_struct* tstruct, bool is_exception) {
 ...
 */
void t_c_generator::generate_struct_definition(ofstream& out,
                                                 t_struct* tstruct,
                                                 bool is_exception,
                                                 bool pointers,
                                                 bool read,
 ...
 * @param tstruct The struct
 */
void t_c_generator::generate_struct_fingerprint(ofstream& out,
                                                  t_struct* tstruct,
                                                  bool is_definition) {
 ...
 */
void t_c_generator::generate_local_reflection(std::ofstream& out,
                                                t_type* ttype,
                                                bool is_definition) {
  if (!gen_dense_) {
    return;
 ...
 */
void t_c_generator::generate_local_reflection_pointer(std::ofstream& out,
                                                        t_type* ttype) {
  if (!gen_dense_) {
    return;
  }
 ...
 */
void t_c_generator::generate_struct_result_writer(ofstream& out,
                                                    t_struct* tstruct,
                                                    bool pointers) {
  string name = tstruct->get_name();
  const vector<t_field*>& fields = tstruct->get_members();
 ...
 * @param tservice The service to generate a header definition for
 */
void t_c_generator::generate_service_interface(t_service* tservice) {
  string extends = "";
 ...
 * @param tservice The service to generate a header definition for
 */
void t_c_generator::generate_service_null(t_service* tservice) {
  string extends = "";
  if (tservice->get_extends() != NULL) {
 ...
 */
void t_c_generator::generate_service_multiface(t_service* tservice) {
 ...
 */
void t_c_generator::generate_function_helpers(t_service* tservice,
 ...
 */
void t_c_generator::generate_process_function(t_service* tservice,
                                                t_function* tfunction) {
  // Open function
  f_service_ <<
 ...
 */
bool t_c_generator::generate_simple_type_limited_reflection(ostream & out, t_type* ttype, string target) {
 ...
 * @param target  The name of the lvalue to reflect onto
 */
bool t_c_generator::generate_type_limited_reflection(t_type* ttype, string target) {
 ...
 */
void t_c_generator::generate_service_limited_reflector(t_service* tservice) {
  // Open function
  f_service_ <<
 ...
 * @param tservice The service to generate a server for.
 */
void t_c_generator::generate_service_skeleton(t_service* tservice) {
  string svcname = tservice->get_name();

  // Service implementation file includes
  string f_skeleton_name = get_out_dir()+svcname+"_server.skeleton.c";

  string ns = namespace_prefix(tservice->get_program()->get_namespace("c"));
 ...
 * @return Namespaces
 */
string t_c_generator::namespace_prefix(string ns) {
  if (ns.size() == 0) {
    return "";
  }
  string result = "";
 ...
 * @return Namespaces
 */
string t_c_generator::namespace_open(string ns) {
  if (ns.size() == 0) {
    return "";
  }
  string result = "";
 ...
 * @return Namespaces
 */
string t_c_generator::namespace_close(string ns) {
  if (ns.size() == 0) {
    return "";
  }
  string result = "}";
 ...
 */
string t_c_generator::type_name(t_type* ttype, bool in_typedef, bool arg) {
  if (ttype->is_base_type()) {
  ...
  if (program != NULL && program != program_) {
    pname =
      class_prefix +
      namespace_prefix(program->get_namespace("c")) +
      ttype->get_name();
  } else {
 ...
 */
string t_c_generator::local_reflection_name(const char* prefix, t_type* ttype, bool external) {
  ttype = get_true_type(ttype);
  ...
  if (external &&
    ...
    nspace = namespace_prefix(ttype->get_program()->get_namespace("c"));
  }

  return nspace + "trlo_" + prefix + "_" + prog + "_" + name;
}

string t_c_generator::get_include_prefix(const t_program& program) const {
