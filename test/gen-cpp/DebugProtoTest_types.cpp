        ...
        if (ftype == ::apache::thrift::protocol::T_MAP) {
          {
            ...
            iprot->readMapBegin(_ktype583, _vtype584, _size582);
            uint32_t _i586;
            for (_i586 = 0; _i586 < _size582; ++_i586)
            {
              int32_t _key587;
              xfer += iprot->readI32(_key587);
              int32_t& _val588 = this->somemap_field[_key587];
              xfer += iprot->readI32(_val588);
            }
            iprot->readMapEnd();
          }
...
uint32_t StructWithASomemap::write(::apache::thrift::protocol::TProtocol* oprot) const {
  ...
  xfer += oprot->writeFieldBegin("somemap_field", ::apache::thrift::protocol::T_MAP, 1);
  {
    ...
    std::map<int32_t, int32_t> ::const_iterator _iter589;
    for (_iter589 = this->somemap_field.begin(); _iter589 != this->somemap_field.end(); ++_iter589)
    {
      xfer += oprot->writeI32(_iter589->first);
      xfer += oprot->writeI32(_iter589->second);
    }
    xfer += oprot->writeMapEnd();
  }
  xfer += oprot->writeFieldEnd();
